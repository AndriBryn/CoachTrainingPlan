...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    const { data: commitData } = await octokit.request('GET /repos/{owner}/{repo}/commits/{branch}', {
      owner,
      repo,
      branch,
    });
    ...
    // Create a new commit
    const { data: commitResponse } = await octokit.request('POST /repos/{owner}/{repo}/git/commits', {
      owner,
      repo,
